...
public class SceneColorControl : MonoBehaviour 
{
	...
	public bool setSkyboxMaterial			= false;
	public bool getCurrentSkyboxMaterial	= false;
	...
	void Update () 
	{
		if(getCurrentSkyboxMaterial){GetSkyboxMaterial();}
		if(setSkyboxMaterial){SetSkyboxMaterial();}
	...
	void SetSkyboxMaterial()
	{
		...
		setSkyboxMaterial = false;
	}
	
	void GetSkyboxMaterial()
	{
		skyMaterial = RenderSettings.skybox;
		getCurrentSkyboxMaterial = false;
	}
	...
	void UpdateSceneColors()
	{
		...
		RenderSettings.fogColor = Vector4.Lerp(defaultFogColor, 0.5f * (fogTint + sceneColor) + 0.2f*(keyLight.color * keyLight.intensity * System.Convert.ToSingle(keyLight.enabled)) * keylightIsDirectional,fogColorInfluence);
